...
export function analyse(company,contacts=[],signals=[],now=new Date()) {
 ...
 const wine=findClaim(evidence,/\b(wines?|vins?|wijn(?:en)?|wein)\b/i,'Wine portfolio documented','portfolio');
 ...
 const importer=findClaim(evidence,/\b(importer|importateur|importeur|wijnimporteur|drankenimporteur|importons|direct[- ]import)\b/i,'Import capability documented');
