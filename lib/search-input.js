...
export function validateInput(body) {
  if(!body||typeof body!=='object'||Array.isArray(body)||Object.keys(body).some(k=>!['market','prospect_type','product_focus','request_id'].includes(k))||
    !markets.includes(body.market)||!types.includes(body.prospect_type)||!products.includes(body.product_focus)||!uuid(body.request_id)) throw new AppError('INVALID_INPUT',400);
  return body;
}
export function guardPost(req) {
  if(!String(req.headers?.['content-type']||'').toLowerCase().startsWith('application/json')) throw new AppError('INVALID_INPUT',415);
