...
export function clusterSources(results) {
 ...
 for(const raw of results.slice(0,70)) {
  ...
  if(!groups.has(host))groups.set(host,{website:`https://${host}/`,pages:[],name:null});
  const g=groups.get(host);if(previous){if(!previous.quote.includes(quote))previous.quote=(previous.quote+' '+quote).slice(0,1600);}else{const page={source_url:url,quote};g.pages.push(page);seen.set(key.href,page);}
