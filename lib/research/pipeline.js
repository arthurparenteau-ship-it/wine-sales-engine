...
export async function runPipeline({input,search,provider,db,signal,now=new Date()}) {
 ...
 let discovery;
 try {discovery=await provider.discover(input,signal,async metrics=>{progress={...progress,...metrics};log(search.id,'provider',progress);});}
 ...
 const clustered=clusterSources(results),accepted=[],rejectedSamples=[],rejections={...clustered.rejections};
 for(const candidate of clustered.candidates){const verified=verifyCandidate(candidate,input);if(verified.candidate&&accepted.length<20)accepted.push(verified.candidate);else{const reason=verified.reason||'candidate_limit';rejections[reason]=(rejections[reason]||0)+1;if(rejectedSamples.length<24)rejectedSamples.push({website:candidate.website,name:candidate.name,titles:candidate.titles,reason});}}
 const candidates=accepted.map(c=>qualifyV2(c,input,now));
 const metrics={engine_version:'intelligence-v2',...progress,...discovery.metrics,raw_results:results.length,unique_candidates:clustered.candidates.length,verified_candidates:accepted.length,accepted_companies:candidates.length,rejected_candidates:clustered.candidates.length-candidates.length,rejection_reasons:rejections,rejected_samples:rejectedSamples,
