...
import {queryPlan,nextQuery,enrichmentQuery,MAX_QUERIES,RESULTS_PER_QUERY} from './queries.js';
...
export function createProvider(env=process.env) {
 ...
 return {name:'brave',async discover(input,signal,onProgress=async()=>{}) {
  const results=[],metrics={provider_request_count:0,raw_results:0,query_yield:[]};
  const request=async(plan)=>{
   const q=plan.query,before=clusterSources(results).candidates.filter(c=>verifyCandidate(c,input).candidate).length,started=Date.now();
   if(metrics.provider_request_count>=MAX_QUERIES||signal.aborted)throw new AppError('TIMEOUT',504);
   const url=new URL('https://api.search.brave.com/res/v1/web/search');
   url.search=new URLSearchParams({q,country:countryCodes[input.market],count:String(RESULTS_PER_QUERY),extra_snippets:'true',search_lang:plan.language||'en',text_decorations:'false',result_filter:'web'});
   ...
   try {
    ...
    const clustered=clusterSources(results),verified=clustered.candidates.filter(c=>verifyCandidate(c,input).candidate).length;
    metrics.query_yield.push({...plan,raw_count:Math.min(rows.length,RESULTS_PER_QUERY),unique_candidates:clustered.candidates.length,verified_count:verified,new_verified:Math.max(0,verified-before),official_candidates:clusterSources(rows).candidates.filter(c=>c.identity).length,duration_ms:Date.now()-started});
  ...
  for(let i=0;i<5;i++) {
   const q=nextQuery(plan,metrics.query_yield);if(!q)break;await request(q);
   if(clusterSources(results).candidates.filter(c=>verifyCandidate(c,input).candidate).length>=20)break;
  }
  const verified=clusterSources(results).candidates.filter(c=>verifyCandidate(c,input).candidate).sort((a,b)=>b.evidence.length-a.evidence.length).slice(0,2);
  for(const candidate of verified)await request({id:'enrich-'+new URL(candidate.website).hostname,language:'en',family:'enrichment',query:enrichmentQuery(new URL(candidate.website).hostname,input)});
