...
export default async function handler(req,res) {
  ...
  try {
    if(req.method==='GET') {
      ...
      return res.status(200).json({success:true,search:row});
    }
    guardPost(req);const input=validateInput(req.body);
    db=database();const started=await db.rpc('wse_begin_search',{p_request_id:input.request_id,p_market:input.market,p_type:input.prospect_type,p_focus:input.product_focus},controller.signal);
    if(started.error)throw new AppError(started.error,started.error==='RATE_LIMITED'?429:409);
    search=started.search;
    if(!started.created)return res.status(search.status==='completed'||search.status==='failed'?200:202).json({success:true,search});
    log(search.id,'pending');
    await db.request(`searches?id=eq.${search.id}&status=eq.pending`,{method:'PATCH',body:{status:'running',updated_at:new Date().toISOString()},prefer:'return=minimal',signal:controller.signal});
    ...
    if(search&&db) {
      ...
      try {
        await db.request(`searches?id=eq.${search.id}&status=in.(pending,running)`,{method:'PATCH',body:{status:'failed',error_code:code,updated_at:new Date().toISOString()},prefer:'return=minimal'});
